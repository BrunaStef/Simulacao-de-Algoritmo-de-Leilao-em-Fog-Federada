...
federation_info = {
    ...
    'mem_usage': [0] * (QUANTITY_FOGS + 1), ## vetor de uso da memória 
    ##'bandwidth': [0] * (QUANTITY_FOGS + 1)
}   
...
def main():
    ...
    check_docker_group_version()  ## Colocamos essa função para ser chamada na main
...
def start_simulation(client):
    ...
    ## RepeatTimer(interval=2, function=update_bandwidth_to_client, args=(client,)).start()
...
def update_cpu_usage(container):
    global cpu_usage, mem_usage, cpu_usage_mutex
    ...
    while True:
        ...
        with cpu_usage_mutex:
            ...
            ## Colocamos essa váriavel para indicar o uso da memória
            mem_usage = retrieve_memory_usage_from_docker_stats(stats)
...
## Função copiadada do data_collector
...
## Função copiadada do data_collector que retorna a memória usada
...
def run_auction(client: mqtt.Client, auction_messages: list):
    ...
    if max_cpu_usage == 0:
        max_cpu_usage = 1
    ## calculando o uso máximo da memória
    max_mem_usage = max(federation_info['mem_usage'])
    if max_mem_usage == 0:
        max_mem_usage = 1
    ...
    for i in range(messages_number):
        ...
        for j in range(QUANTITY_FOGS):
            if j != FOG_ID-1:
                ...
                normalized_mem = 100 * federation_info['mem_usage'][j] / max_mem_usage
                current_benefit = abs(math.log10(1/normalized_actual_latency)) + abs(math.log10(1/normalized_mem)) + abs(math.log10(1/normalized_cpu))
...
def handle_response_federation_info(parsed_message):
    ...
    federation_info['mem_usage'][response_fog] = parsed_message['mem_usage']
    ## federation_info['bandwidth'][response_fog] = parsed_message['bandwidth']
